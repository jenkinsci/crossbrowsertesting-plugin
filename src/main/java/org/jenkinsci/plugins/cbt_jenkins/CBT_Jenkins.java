...
public class CBT_Jenkins extends Builder implements Serializable, BuildStep {
    ...
    private static Queue<String> operatingSystems = new LinkedList<String>();
    private static Queue<String> browsers = new LinkedList<String>();
    private static Queue<String> resolutions = new LinkedList<String>();
    ...
    public boolean perform(AbstractBuild<?,?> build, Launcher launcher, BuildListener listener) throws IOException, InterruptedException {
    	...
    	try {
			...
			listener.getLogger().println("ERROR: " + iae.getMessage());
			return false;
		}catch(Exception e) {
...
*/
        ...
        public ListBoxModel doFillOperatingSystemItems() {
        	...
        	if (!operatingSystems.isEmpty()) {
        		operatingSystem = operatingSystems.poll();
        	}
        	
        	int index = -1;
            ListBoxModel items = new ListBoxModel();
            for (int i=0 ; i<browserList.configurations.size() ; i++) {
            	Configuration config = browserList.configurations.get(i);
                items.add(config.getName());
                if (operatingSystem.equals(config.getName())) {
                	index = i;
                }
            }
            if (index > 0) {
        ...
        public ListBoxModel doFillBrowserItems(@QueryParameter String operatingSystem) {
            ...
            for (int i=0 ; i<config.browsers.size() ; i++) {
                ...
                if (browser.equals(configBrowser.getName())) {
                	index = i;
                }
        	}
            if (index > 0) {
        ...
        public ListBoxModel doFillResolutionItems(@QueryParameter String operatingSystem) {
            ...
            for (int i=0 ; i<config.resolutions.size() ; i++) {
                ...
                if (resolution.equals(configResolution.getName())) {
                	index = i;
                }
        	}
            if (index > 0) {
